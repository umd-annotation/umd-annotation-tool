                ...
                if 'message' in data and isinstance(data['message'], dict):
                    ...
                    if 'type' in data['message']:
                        ...
                        if data['message']['type'] == 'norm_occurrence' and data['message']['name']:
                            if str(data['message']['name']) in normMap:
...
def main_script(input_folder):
    for file in os.listdir(input_folder):
        if file.endswith(".json"):
            ...
            output_file = os.path.join(input_folder, file.replace(".json", "_tracks.json"))
            output = load_json_file(input_file)
            turns = process_outputjson(output)
            print(turns)
            # with open(os.path.join(input_folder, 'turns.json'), 'w') as outfile:
