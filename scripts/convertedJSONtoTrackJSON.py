...
def convert_converted_to_track_json(input_file,):
    ...
    turns = process_outputjson(data)
    trackJSON = convert_output_to_tracks(turns)
    output_file = input_file.replace('_converted.json', '_tracks.json')
    with open(output_file, 'w') as outfile:
        json.dump(trackJSON, outfile, indent=2)
        click.echo(f"Track JSON data saved to {output_file}")


if __name__ == '__main__':
    convert_converted_to_track_json()
